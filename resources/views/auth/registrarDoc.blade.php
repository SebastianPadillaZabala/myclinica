...
<div class="h-screen bg-gradient-to-b opacity-85 from-green-500 to-green-400 flex justify-center items-center w-full">
  <form method="POST" action="{{ route('creardoctor') }}">
  @csrf
    <div class="bg-white px-10 py-8 rounded-xl w-screen shadow-md max-w-sm">
      <div class="card-body">
        ...
        <label for="email" class="block mb-1 text-gray-600 font-semibold">Especialidad</label>
                            <select name="select">
                                <option value="">Selecciona una Especialidad</option>
                                @foreach ($esp as $e)
                                    <option value="<?= $e->id_esp ?>"><?= $e->nombreEsp?></option>
                                @endforeach
                              </select>
